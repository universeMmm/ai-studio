 ...
 * @returns The localized full plan name (e.g., "AI Studio Pro", "AI Studio Free")
 */
export function getChatPlanName(chatEntitlement: ChatEntitlement): string {
	switch (chatEntitlement) {
		case ChatEntitlement.EDU:
			return localize('plan.eduName', 'AI Studio Student');
		case ChatEntitlement.Pro:
			return localize('plan.proName', 'AI Studio Pro');
		case ChatEntitlement.ProPlus:
			return localize('plan.proPlusName', 'AI Studio Pro+');
		case ChatEntitlement.Max:
			return localize('plan.maxName', 'AI Studio Max');
		case ChatEntitlement.Business:
			return localize('plan.businessName', 'AI Studio Business');
		case ChatEntitlement.Enterprise:
			return localize('plan.enterpriseName', 'AI Studio Enterprise');
		default:
			return localize('plan.freeName', 'AI Studio Free');
	}
}
	...
	private async onUnknownSignUpError(detail: string, logMessage: string): Promise<boolean> {
		...
		if (!this.lifecycleService.willShutdown) {
			const { confirmed } = await this.dialogService.confirm({
				type: Severity.Error,
				message: localize('unknownSignUpError', "An error occurred while signing up for the AI Studio Free plan. Would you like to try again?"),
				detail,
	...
	private onUnprocessableSignUpError(logMessage: string, logDetails: string): void {
		...
		if (!this.lifecycleService.willShutdown) {
			this.dialogService.prompt({
				type: Severity.Error,
				message: localize('unprocessableSignUpError', "An error occurred while signing up for the AI Studio Free plan."),
				detail: logDetails,
				buttons: [
					{
