 ...
 *
 * Steps:
 * 1. Sign In — sessions-style sign-in hero with AI Studio, Google, and Apple options
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		copilotDisclaimer.append(localize('onboarding.signIn.disclaimer.aiStudioPrefix', ". {0} AI Studio may show ", defaultChat.provider.default.name));
	...
	private _renderAgentSessionsStep(container: HTMLElement): void {
		...
		this._createFeatureCard(moreGrid, Codicon.rocket,
			...
			localize('onboarding.sessions.runAnywhere.desc', "Run agents locally for interactive work, in the background with AI Studio CLI, or in the cloud with cloud agents that open a pull request your team can review."));

		this._createFeatureCard(moreGrid, Codicon.settingsGear,
			localize('onboarding.sessions.customize', "Customize Your Agents"),
			localize('onboarding.sessions.customize.desc', "Tailor AI Studio to your project with custom instructions and agents, skills, reusable prompts, and MCP servers that connect to the tools and context you rely on."));
