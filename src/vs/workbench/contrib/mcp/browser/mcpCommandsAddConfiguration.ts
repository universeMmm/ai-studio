...
const enum AddConfigurationCopilotCommand {
	...
	IsSupported = 'aiStudio.chat.mcp.setup.check',

	/** Takes an npm/pip package name, validates its owner. */
	ValidatePackage = 'aiStudio.chat.mcp.setup.validatePackage',

	/** Returns the resolved MCP configuration. */
	StartFlow = 'aiStudio.chat.mcp.setup.flow',
}
...
export class McpAddConfigurationCommand {
	...
	private async getServerType(): Promise<AddConfigurationType | undefined> {
		...
		if (aiSupported) {
			...
			items.push(
				{ type: 'separator', label: localize('mcp.serverType.aiStudio', "Model-Assisted") },
				...elligableTypes
			);
		}
