...
export class CopilotCliAgentPluginDiscovery extends AbstractAgentPluginDiscovery {
	...
	private async _promptRemove(resource: URI): Promise<void> {
		const { confirmed } = await this._dialogService.confirm({
			message: localize('aiStudioCliPlugin.remove.confirm', "This plugin was installed by the AI Studio CLI. Remove it from disk?"),
			detail: localize('aiStudioCliPlugin.remove.detail', "The plugin directory '{0}' will be moved to the trash. You can reinstall it later via the AI Studio CLI.", resource.fsPath),
			primaryButton: localize('aiStudioCliPlugin.remove.primary', "Remove"),
		});
		if (!confirmed) {
			return;
		}
