...
export class PromptValidator {
	...
	private async validateSkillAttributes(promptAST: ParsedPromptFile, promptType: PromptsType, report: (markers: IMarkerData) => void): Promise<void> {
		...
		if (contextAttribute && contextAttribute.value.type === 'scalar' && contextAttribute.value.value.trim() === 'fork') {
			const skillToolEnabled = this.configurationService.getValue<boolean>('aiStudio.chat.skillTool.enabled');
			if (!skillToolEnabled) {
				report(toMarker(
					localize('promptValidator.contextForkNotSupported', "The 'context: fork' attribute requires the skill tool to be enabled (aiStudio.chat.skillTool.enabled)."),
				...
				switch (promptType) {
					...
					case PromptsType.agent:
						if (target === Target.GitHubCopilot) {
							report(toMarker(localize('promptValidator.unknownAttribute.github-agent', "Attribute '{0}' is not supported in custom AI Studio agent files. Supported: {1}.", attribute.key, supportedNames.value), attribute.range, MarkerSeverity.Hint, [MarkerTag.Unnecessary]));
						} else if (target === Target.Claude) {
	...
	private validateHookCommand(item: IValue, target: Target, report: (markers: IMarkerData) => void): void {
		...
		if (!hasCommandField) {
			if (isCopilotCli) {
				report(toMarker(localize('promptValidator.hookMissingAiStudioCommand', "Hook command must specify at least one of 'bash' or 'powershell'."), item.range, MarkerSeverity.Error));
			} else {
		...
		case PromptsType.agent:
			switch (attributeName) {
				...
				case PromptHeaderAttributes.target:
					return localize('promptHeader.agent.target', 'The target to which the header attributes like tools apply to. Possible values are `ai-studio` and `vscode`.');
...
// The list of tools known to be used by AI Studio custom agents
export const knownGithubCopilotTools = [
	{ name: SpecedToolAliases.execute, description: localize('aiStudio.execute', 'Execute commands') },
	{ name: SpecedToolAliases.read, description: localize('aiStudio.read', 'Read files') },
	{ name: SpecedToolAliases.edit, description: localize('aiStudio.edit', 'Edit files') },
	{ name: SpecedToolAliases.search, description: localize('aiStudio.search', 'Search files') },
	{ name: SpecedToolAliases.agent, description: localize('aiStudio.agent', 'Use subagents') },
];
...
export const knownClaudeModels = [
	{ name: 'sonnet', description: localize('claude.sonnet', 'Latest Claude Sonnet'), modelEquivalent: 'Claude Sonnet 4.5 (ai-studio)' },
	{ name: 'opus', description: localize('claude.opus', 'Latest Claude Opus'), modelEquivalent: 'Claude Opus 4.6 (ai-studio)' },
	{ name: 'haiku', description: localize('claude.haiku', 'Latest Claude Haiku, fast for simple tasks'), modelEquivalent: 'Claude Haiku 4.5 (ai-studio)' },
