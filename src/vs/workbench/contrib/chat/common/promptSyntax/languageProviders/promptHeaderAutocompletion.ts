				...
				if (value.type === 'sequence') {
					...
					const getValues = async () => {
						if (target === Target.GitHubCopilot || this.environmentService.isSessionsWindow) {
							// for AI Studio targets and the Sessions Window, we only suggest the known set of tools that are supported by AI Studio, instead of all tools that the user has defined, because many tools won't work in these contexts and it would be frustrating for users to select a tool that doesn't work
