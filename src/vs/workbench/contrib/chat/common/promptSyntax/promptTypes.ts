 ...
 */
export const AGENT_DEBUG_LOG_ENABLED_SETTING = 'aiStudio.chat.agentDebugLog.enabled';

/**
 * Configuration key for enabling file logging for the agent debug log.
 */
export const AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING = 'aiStudio.chat.agentDebugLog.fileLogging.enabled';
...
/**
 * URI scheme used by the AI Studio extension for built-in skills.
 */
...
export function getSourceDescription(source: PromptFileSource): string | undefined {
	switch (source) {
		...
		case PromptFileSource.GitHubWorkspace:
			return localize('source.githubWorkspace', "Workspace (only used by AI Studio agents)");
		case PromptFileSource.CopilotPersonal:
			return localize('source.aiStudioPersonal', "Global (only used by AI Studio agents)");
