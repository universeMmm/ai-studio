...
/**
 * JSON Schema for AI Studio hook configuration files.
...
export const hookFileSchema: IJSONSchema = {
	...
	type: 'object',
	description: nls.localize('hookFile.description', 'AI Studio hook configuration file. Hooks enable executing custom shell commands at strategic points in an agent\'s workflow.'),
	additionalProperties: true,
	required: ['hooks'],
	properties: {
		hooks: {
	...
	// whether the file uses the AI Studio CLI format (detected by the "version" field).
	if: {
		required: ['version'],
		properties: {
			version: { type: 'number' }
	...
	then: {
		// AI Studio CLI format: camelCase hook names, bash/powershell/timeoutSec fields
		properties: {
			version: {
				type: 'number',
 ...
 * Only matches exact enum values. For tool-specific naming conventions (e.g., Claude, AI Studio CLI),
 ...
 *
 * This allows AI Studio format to handle Claude-style entries if pasted.
 ...
 * This ensures compatibility when Claude-style hooks are pasted into AI Studio format.
 */
