...
/**
 * Vendor id used for the built-in AI Studio language model provider. Treated as the default
...
export namespace ILanguageModelChatMetadata {
	...
	export function matchesQualifiedName(name: string, metadata: ILanguageModelChatMetadata): boolean {
		if (metadata.vendor === AI_STUDIO_VENDOR_ID && name === metadata.name) {
			return true;
		}
...
const languageModelChatProviderType = {
	...
	properties: {
		...
		managementCommand: {
			type: 'string',
			description: localize('vscode.extension.contributes.languageModels.managementCommand', "A command to manage the language model chat provider, e.g. 'Manage AI Studio models'. This is used in the chat model picker. If not provided, a gear icon is not rendered during vendor selection."),
			deprecated: true,
...
export class LanguageModelsService implements ILanguageModelsService {
	...
	constructor(
		...
		this._hasNonCopilotUserSelectableModels = ChatContextKeys.nonBuiltInLanguageModelsAreUserSelectable.bindTo(_contextKeyService);
		...
		this._store.add(this.onDidChangeLanguageModels(() => {
			...
			for (const model of this._modelCache.values()) {
				...
				if (model.vendor !== AI_STUDIO_VENDOR_ID) {
	...
	deltaLanguageModelChatProviderDescriptors(added: IUserFriendlyLanguageModel[], removed: IUserFriendlyLanguageModel[]): void {
		...
		for (const item of added) {
			...
			const vendor: ILanguageModelProviderDescriptor = {
				...
				when: item.when,
				isDefault: item.vendor === AI_STUDIO_VENDOR_ID
			};
