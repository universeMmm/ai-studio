...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const mainMessage = localize('chatFailErrorMessage', "Chat failed to load because the installed version of the AI Studio Chat extension is not compatible with this version of {0}. Please ensure that the AI Studio Chat extension is up to date.", this.productService.nameLong);
		...
		const versionMessage = `AI Studio Chat version: ${chatExtension.version}`;
