...
// - Remote AH: `remote-<auth>-aiStudioCli:/<id>`      (remoteAgentHostSessionTypeId)
// - EH CLI ext: `aiStudioCli:/<id>`                   (extension's own session type)
const AI_STUDIO_CLI_PROVIDER = 'aiStudioCli';
export const AI_STUDIO_CLI_LOCAL_AH_SCHEME = `agent-host-${AI_STUDIO_CLI_PROVIDER}`;
export const AI_STUDIO_CLI_EH_SCHEME = AI_STUDIO_CLI_PROVIDER;
const REMOTE_PROVIDER_PREFIX = 'remote-';
const REMOTE_PROVIDER_SUFFIX = `-${AI_STUDIO_CLI_PROVIDER}`;

/**
 * Builds the local `events.jsonl` URI under `~/.ai-studio/session-state/<rawId>/`.
 *
 * Used for both the local Agent Host AI Studio CLI provider and the
 * extension-host AI Studio CLI provider, which share the same on-disk layout
 * and the same chat session URI shape (`aiStudioCli:/<rawId>`).
 */
export function buildLocalEventsUri(userHome: URI, rawSessionId: string): URI {
	return joinPath(userHome, '.ai-studio', 'session-state', rawSessionId, 'events.jsonl');
}
...
export function buildRemoteEventsUri(connection: IRemoteAgentHostConnectionInfo, rawSessionId: string): URI | undefined {
	...
	const remoteFileUri = URI.from({
		scheme: 'file',
		path: `${trimmed}/.ai-studio/session-state/${rawSessionId}/events.jsonl`,
	});
 ...
 * of the form `remote-<authority>-aiStudioCli`. Returns `undefined` for
 * any other scheme, including the local `aiStudioCli` scheme.
 */
 ...
 * `events.jsonl` URI for the corresponding AI Studio CLI session, or
...
export function resolveEventsUri(
	...
	if (sessionResource.scheme === AI_STUDIO_CLI_LOCAL_AH_SCHEME || sessionResource.scheme === AI_STUDIO_CLI_EH_SCHEME) {
