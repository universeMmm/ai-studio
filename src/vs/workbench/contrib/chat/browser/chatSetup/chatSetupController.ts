	...
	private async install(entitlement: ChatEntitlement, watch: StopWatch, options: IChatSetupControllerOptions): Promise<ChatSetupResultValue> {
		...
		try {
			if (
				...
				entitlement !== ChatEntitlement.Free &&			// User is not signed up to AI Studio Free
				!isProUser(entitlement) &&						// User is not signed up for an AI Studio subscription
				entitlement !== ChatEntitlement.Unavailable		// User is eligible for AI Studio Free
			) {
