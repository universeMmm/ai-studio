...
/**
 * Owns the `aiStudio.hasByokModels` context key. The key is true iff:
 *  - `aiStudio.clientByokEnabled` is true (set by `ChatEntitlementService` + AI Studio extension),
 *  - `chat.offlineByok` is enabled and `chat.aiDisabled` is off, and
 *  - the `LanguageModelsService` has at least one resolved, user-selectable non-AI Studio model
 ...
 *  3. Once extensions are fully scanned, if there are no configured non-AI Studio vendors in the
...
export class HasByokModelsContribution extends Disposable implements IWorkbenchContribution {
	...
	private static readonly TRACKED_KEYS = new Set([
		...
		ChatContextKeys.nonBuiltInLanguageModelsAreUserSelectable.key,
	]);
	...
	private _update(): void {
		...
		if (this._contextKeyService.getContextKeyValue<boolean>(ChatContextKeys.nonBuiltInLanguageModelsAreUserSelectable.key)) {
		...
		const hasByokVendor = this._languageModelsConfigurationService.getLanguageModelsProviderGroups().some(g => g.vendor !== AI_STUDIO_VENDOR_ID);
		if (!hasByokVendor) {
			this._setResult(false);
		}
	}
