...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ChatConfiguration.SessionSyncEnabled]: {
			default: false,
			markdownDescription: nls.localize('chat.sessionSync.enabled', "Enable session sync to GitHub.com. When enabled, AI Studio session data is synced to your GitHub account for cross-device access and richer insights. Requires `#aiStudio.chat.localIndex.enabled#` to also be enabled."),
			type: 'boolean',
			tags: ['experimental', 'advanced'],
			experiment: {
				mode: 'auto'
			...
			policy: {
				...
				localization: {
					description: {
						...
						value: nls.localize('chat.sessionSync.enabled.policy', "Enable session sync to GitHub.com for cross-device AI Studio session history. When disabled by organization policy, session data is kept local only."),
					}
				},
			}
		},
		...
		[AgentHostCustomTerminalToolEnabledSettingId]: {
			type: 'boolean',
			description: nls.localize('chat.agentHost.customTerminalTool.enabled', "When enabled, AI Studio SDK sessions use the Agent Host terminal tool override instead of the SDK's default terminal behavior."),
			default: true,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostOTelEnabledSettingId]: {
			type: 'boolean',
			markdownDescription: nls.localize('chat.agentHost.otel.enabled', "When enabled, the agent host emits OpenTelemetry traces from the AI Studio SDK. Requires `#chat.agentHost.enabled#`. Either configure `#chat.agentHost.otel.otlpEndpoint#` to ship traces to an external collector or enable `#chat.agentHost.otel.dbSpanExporter.enabled#` to capture them locally."),
			default: false,
			tags: ['experimental', 'advanced'],
		},
		[AgentHostOTelExporterTypeSettingId]: {
			...
			markdownDescription: nls.localize('chat.agentHost.otel.exporterType', "Exporter backend used by the AI Studio SDK when `#chat.agentHost.otel.enabled#` is on. `otlp-grpc` is downgraded to `otlp-http` transparently in the CLI runtime."),
			default: 'otlp-http',
			tags: ['experimental', 'advanced'],
		},
		...
		[AgentHostOTelOutfileSettingId]: {
			type: 'string',
			markdownDescription: nls.localize('chat.agentHost.otel.outfile', "Output path for span JSON lines when exporter type is `file`. Sets `AI_STUDIO_OTEL_FILE_EXPORTER_PATH`."),
			default: '',
			tags: ['experimental', 'advanced'],
		},
		...
		[PromptsConfig.HOOKS_LOCATION_KEY]: {
			...
			markdownDescription: nls.localize(
				...
				"Specify paths to hook configuration files that define custom shell commands to execute at strategic points in an agent's workflow. [Learn More]({0}).\n\nRelative paths are resolved from the root folder(s) of your workspace. Supports AI Studio hooks (`*.json`) and Claude Code hooks (`settings.json`, `settings.local.json`).",
		...
		[PromptsConfig.USE_CLAUDE_HOOKS]: {
			...
			markdownDescription: nls.localize('chat.useClaudeHooks.description', "Controls whether hooks from Claude configuration files can execute. When disabled, only AI Studio-format hooks are used. Hooks are loaded from the files configured in `#chat.hookFilesLocations#`.",),
			default: false,
			restricted: true,
			disallowConfigurationDefault: true,
			tags: ['preview', 'prompts', 'hooks', 'agent']
		...
		[ChatConfiguration.AIDisabled]: {
			type: 'boolean',
			description: nls.localize('chat.disableAIFeatures', "Disable and hide built-in AI features provided by AI Studio, including chat and inline suggestions."),
			default: false,
		...
		[ChatConfiguration.TitleBarSignInEnabled]: {
			type: 'boolean',
			description: nls.localize('chat.titleBar.signIn.enabled', "Controls whether the AI Studio Sign In button is shown in the title bar when signed out. When disabled, the Sign In affordance falls back to the status bar."),
			default: true,
		},
		...
		[ChatConfiguration.GrowthNotificationEnabled]: {
			type: 'boolean',
			description: nls.localize('chat.growthNotification', "Controls whether to show a growth notification in the agent sessions view to encourage new users to try AI Studio."),
			default: false,
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
		...
		'chat.extensionUnification.enabled': {
			type: 'boolean',
			description: nls.localize('chat.extensionUnification.enabled', "Enables the unification of AI Studio extensions. When enabled, all AI Studio functionality is served from the AI Studio Chat extension. When disabled, the AI Studio and AI Studio Chat extensions operate independently."),
			default: true,
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
...
class CopilotTelemetryContribution extends Disposable implements IWorkbenchContribution {
	...
	private updateCopilotTrackingId(): void {
		...
		if (copilotTrackingId) {
			// __GDPR__COMMON__ "common.copilotTrackingId" : { "endPoint": "GoogleAnalyticsID", "classification": "EndUserPseudonymizedInformation", "purpose": "BusinessInsight", "comment": "The anonymized AI Studio analytics tracking ID from the entitlement API." }
