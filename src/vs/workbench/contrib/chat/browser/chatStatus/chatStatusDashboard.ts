...
export class ChatStatusDashboard extends DomWidget {
	...
	private render(): void {
		...
		if (hasUsageSection && !this.options?.compactQuotaLayout) {
			...
			const header = this.renderHeader(headerHost, this._store, planName, toAction({
				...
				label: localize('quotaLabel', "Manage AI Studio Settings"),
				tooltip: localize('quotaTooltip', "Manage AI Studio Settings"),
	...
	private renderSetupSection(): void {
		...
		if (newUser && anonymousUser) {
			descriptionText = new MarkdownString(localize({ key: 'activeDescriptionAnonymous', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with {0} AI Studio, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3})", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl), { isTrusted: true });
			descriptionClass = `${descriptionClass}.terms`;
		} else if (newUser) {
			descriptionText = localize('activateDescription', "Set up AI Studio to use AI features.");
		} else if (anonymousUser) {
			descriptionText = localize('enableMoreDescription', "Sign in to enable more AI Studio features.");
		} else if (disabled) {
			descriptionText = localize('enableDescription', "Enable AI Studio to use AI features.");
		} else {
			descriptionText = localize('signInDescription', "Sign in to use AI Studio features.");
		}
		...
		if (newUser) {
			...
			buttonLabel = localize('enableAiStudioButton', "Enable AI Features");
		} else {
			buttonLabel = localize('signInToUseAIFeatures', "Sign in to use AI Studio");
		}
		...
		const update = () => {
			...
			if (maxUsedPercentage >= 100 && additionalUsageEnabled) {
				...
				calloutText.textContent = isEnterpriseUser
					? localize('quotaPausedEnterprise', "AI Studio is paused until the limit resets. Contact your administrator for more information.")
					: localize('quotaPaused', "AI Studio is paused until the limit resets.");
			} else if (maxUsedPercentage >= 75 && !additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaWarningEnterprise', "AI Studio will pause when the limit is reached. Contact your administrator for more information.")
					: localize('quotaWarning', "AI Studio will pause when the limit is reached.");
			} else {
				quotaCallout.style.display = 'none';
			}
