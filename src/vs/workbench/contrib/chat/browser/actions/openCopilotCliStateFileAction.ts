...
/**
 * Shared implementation of "Open AI Studio CLI State File". Resolves the
...
export async function openCopilotCliStateFile(
	...
	switch (result.kind) {
		case 'ok':
			...
			return;
		case 'no-session':
			notificationService.info(localize('openSessionEventsFile.noSession', "No AI Studio CLI session is active."));
			return;
		case 'unsupported-scheme':
			notificationService.info(localize('openSessionEventsFile.unsupported', "The active chat session is not an AI Studio CLI session."));
			return;
...
export class OpenCopilotCliStateFileAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2('openSessionEventsFile', "Open AI Studio CLI State File"),
			f1: true,
