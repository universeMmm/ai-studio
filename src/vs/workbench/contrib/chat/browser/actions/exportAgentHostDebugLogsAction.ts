...
import { AI_STUDIO_CLI_LOCAL_AH_SCHEME, parseRemoteAuthorityFromScheme, resolveEventsUri } from '../copilotCliEventsUri.js';
 ...
 * AI Studio CLI session events file (if available), the window/shared/agent-host
 ...
 * find the active AI Studio CLI chat session. Suitable for vscode where the
 ...
 * or `undefined` if the scheme does not belong to a AI Studio CLI agent-host
...
export function toActiveAgentHostSession(resource: URI, title: string | undefined): IActiveAgentHostSessionForExport | undefined {
	if (resource.scheme === AI_STUDIO_CLI_LOCAL_AH_SCHEME) {
