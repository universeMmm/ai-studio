...
export class AgentHostContribution extends Disposable implements IWorkbenchContribution {
	...
	private _registerAgent(agent: AgentInfo): void {
		...
		// (e.g. "AI Studio CLI"), so suffix with "- Agent Host" to disambiguate.
		...
		// Distinguish from the extension-host AI Studio CLI harness, which
		// registers under the same `AI Studio CLI` displayName via the chat
