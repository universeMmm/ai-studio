...
class ChatCommandLineHandler extends Disposable {
	...
	private async prompt(args: typeof this.environmentService.args.chat): Promise<void> {
		...
		const trusted = await this.workspaceTrustRequestService.requestWorkspaceTrust({
			message: localize('aiStudioWorkspaceTrust', "AI features are currently only supported in trusted workspaces.")
		});
...
function getAgentInfo(rootState: RootState | Error | undefined): AgentInfo | undefined {
 ...
 * `agent-host-${agent.provider}` (e.g. `agent-host-aiStudioCli`) only after it
 * surfaces an `AgentInfo` via `rootState`. This is asynchronous, so the static
 * `agent-host-ai-studio` umbrella commands need to wait for that registration
...
async function resolveAgentHostSessionType(agentHostService: IAgentHostService): Promise<string> {
	const agent = getAgentInfo(agentHostService.rootState.value);
	if (agent) {
	...
	const waitForAgent = new Promise<AgentInfo | undefined>(res => {
		const sub = agentHostService.rootState.onDidChange(state => {
			const found = getAgentInfo(state);
			if (found) {
				sub.dispose();
				res(found);
			}
...
// The dynamic per-agent commands (e.g. `agent-host-ai-studio`) are only
