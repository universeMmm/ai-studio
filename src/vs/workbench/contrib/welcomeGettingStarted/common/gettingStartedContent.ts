...
export function copilotSettingsMessage(manageSettingsUrl: string): string {
	return localize({ key: 'settings', comment: ['{Locked="["}', '{Locked="]({0})"}', '{Locked="]({1})"}'] }, "{0} AI Studio may show [public code]({1}) suggestions and use your data to improve the product. You can change these [settings]({2}) anytime.", defaultChat.provider.default.name, defaultChat.publicCodeMatchesUrl, manageSettingsUrl);
}
...
const AiStudioStepTitle = localize('gettingStarted.aiStudioSetup.title', "Use AI features with AI Studio for free");
const AiStudioDescription = localize({ key: 'gettingStarted.aiStudioSetup.description', comment: ['{Locked="["}', '{Locked="]({0})"}'] }, "You can use [AI Studio]({0}) to generate code across multiple files, fix errors, ask questions about your code, and much more using natural language.", defaultChat.documentationUrl ?? '');
const AiStudioTermsString = localize({ key: 'gettingStarted.aiStudioSetup.terms', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with {0} AI Studio, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3})", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl);
const AiStudioAnonymousButton = Button(localize('setupAiStudioButton.setup', "Use AI Features"), `command:workbench.action.chat.triggerSetupAnonymousWithoutDialog`);
const AiStudioSignedOutButton = Button(localize('setupAiStudioButton.setup', "Use AI Features"), `command:workbench.action.chat.triggerSetup`);
const AiStudioSignedInButton = Button(localize('setupAiStudioButton.setup', "Use AI Features"), `command:workbench.action.chat.triggerSetup`);
const AiStudioCompleteButton = Button(localize('setupAiStudioButton.chatWithAIStudio', "Start to Chat"), 'command:workbench.action.chat.open');

function createAiStudioSetupStep(id: string, button: string, when: string, includeTerms: boolean): BuiltinGettingStartedStep {
	...
	return {
		id,
		title: AiStudioStepTitle,
		description,
		when: `${when} && !chatSetupHidden && !chatSetupDisabledInWorkspace`,
		media: {
			type: 'svg', altText: 'VS Code AI Studio multi file edits', path: 'multi-file-edits.svg'
		},
	};
}
...
export const walkthroughs: GettingStartedWalkthroughContent = [
	{
		...
		content: {
			type: 'steps',
			steps: [
				createAiStudioSetupStep('AIStudioSetupAnonymous', AiStudioAnonymousButton, 'chatAnonymous && !chatSetupCompleted', true),
				createAiStudioSetupStep('AIStudioSetupSignedOut', AiStudioSignedOutButton, 'chatEntitlementSignedOut && !chatAnonymous && !aiStudio.hasByokModels', false),
				createAiStudioSetupStep('AIStudioSetupComplete', AiStudioCompleteButton, 'chatSetupCompleted && !chatSetupDisabled && (chatAnonymous || chatPlanPro || chatPlanProPlus || chatPlanMax || chatPlanBusiness || chatPlanEnterprise || chatPlanFree)', false),
				createAiStudioSetupStep('AIStudioSetupSignedIn', AiStudioSignedInButton, '!chatEntitlementSignedOut && (!chatSetupCompleted || chatSetupDisabled || chatPlanCanSignUp)', false),
				{
					id: 'pickColorTheme',
