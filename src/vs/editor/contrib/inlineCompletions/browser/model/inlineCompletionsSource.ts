...
import { DataChannelForwardingTelemetryService, forwardToChannelIf, isAgentHostLikeExtension } from '../../../../../platform/dataChannel/browser/forwardingTelemetryService.js';
...
export class InlineCompletionsSource extends Disposable {
	...
	private _sendInlineCompletionsRequestTelemetry(
		...
		if (!requestResponseInfo.providers.some(p => isAgentHostLikeExtension(p.providerId?.extensionId))) {
			return;
		}
		...
		const emptyEndOfLifeEvent: InlineCompletionEndOfLifeEvent = {
			...
			...forwardToChannelIf(requestResponseInfo.providers.some(p => isAgentHostLikeExtension(p.providerId?.extensionId))),
