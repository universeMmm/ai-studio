...
// agentHost 剥离：本地桩代码
const AGENT_HOST_SKILL_BUTTON_UPDATE_PR_ID = 'workbench.action.agentSessions.runSkill.updatePR';
const isAgentHostSkillButtonId = (_actionId: string): boolean => false;
...
class ChangesButtonBarWidget extends Disposable {
	...
	private _getButtonConfiguration(action: IAction, outgoingChanges: number, reviewState: { isLoading: boolean; commentCount: number | undefined }, hasGitOperationInProgress: boolean, runningLabelObs: IObservable<string | IMarkdownString | undefined>): { showIcon: boolean; showLabel: boolean; isSecondary?: boolean; customLabel?: string | IMarkdownString; customLabelObs?: IObservable<string | IMarkdownString | undefined>; customClass?: string } | undefined {
		if (
			action.id === 'aiStudio.sessions.commit' ||
			action.id === 'aiStudio.chat.createPullRequestCopilotCLIAgentSession.createPR'
		) {
			if (!hasGitOperationInProgress) {
		...
		if (
			action.id === 'aiStudio.sessions.sync' ||
			action.id === 'aiStudio.sessions.commitAndSync'
		) {
		...
		if (
			action.id === 'aiStudio.claude.sessions.sync' ||
		...
		if (
			...
			action.id === 'aiStudio.chat.openPullRequestCopilotCLIAgentSession.openPR'
		) {
			return { showIcon: true, showLabel: false, isSecondary: true };
		}
		...
		if (
			action.id === 'aiStudio.chat.createPullRequestCopilotCLIAgentSession.createPR' ||
			action.id === 'aiStudio.chat.mergeCopilotCLIAgentSessionChanges.merge' ||
			action.id === 'aiStudio.chat.checkoutPullRequestReroute' ||
			action.id === 'pr.checkoutFromChat' ||
			action.id === 'aiStudio.sessions.initializeRepository' ||
			action.id === 'aiStudio.claude.sessions.initializeRepository' ||
			action.id === 'aiStudio.claude.sessions.commit' ||
			action.id === 'aiStudio.claude.sessions.commitAndSync' ||
