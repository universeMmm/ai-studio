 ...
 * skill toolbar buttons and to suppress the AI Studio CLI extension's own
 ...
 * `agent-host-*` session.
 *
 * They mirror the buttons the AI Studio CLI extension contributes for its own
 ...
 * as the AI Studio CLI extension's Sync PR button. Exported so the changes
...
function registerAgentHostSkillButton(spec: IAgentHostSkillButtonSpec): void {
	registerAction2(class extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			// between the AI Studio CLI extension and the local/remote
