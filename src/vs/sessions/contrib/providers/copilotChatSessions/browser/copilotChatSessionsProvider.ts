...
// agentHost 剥离：本地桩代码
const SessionConfigKey = { Isolation: 'session.isolation', Branch: 'session.branch' } as const;
...
export const CopilotCloudSessionType: ISessionType = {
	...
	label: localize('aiStudioCloud', "Cloud"),
	icon: Codicon.cloud,
};
...
const OPEN_REPO_COMMAND = 'aiStudio.chat.cloudSessions.openRepository';

/** Provider ID for the AI Studio Chat Sessions provider. */
...
export const AI_STUDIO_MULTI_CHAT_SETTING = 'sessions.aiStudio.multiChatSessions';
...
export class CopilotChatSessionsProvider extends Disposable implements ISessionsProvider {
	...
	readonly label = localize('aiStudioChatSessionsProvider', "AI Studio Chat");
	...
	constructor(
		...
		this._multiChatEnabled = this.configurationService.getValue<boolean>(AI_STUDIO_MULTI_CHAT_SETTING) ?? true;
	...
	async renameChat(sessionId: string, chatUri: URI, title: string): Promise<void> {
		...
		if (agentSession?.providerType === CopilotCLISessionType.id) {
			await this.commandService.executeCommand('aiStudio.cli.sessions.setTitle', { resource: chatUri }, title);
			return;
		}
		if (agentSession?.providerType === AgentSessionProviders.Claude) {
			await this.commandService.executeCommand('aiStudio.claude.sessions.rename', { resource: chatUri }, title);
			return;
		}
	...
	private async _deleteAgentSessions(agentSessions: IAgentSession[]): Promise<void> {
		...
		if (cliSessionItems.length > 0) {
			await this.commandService.executeCommand('agents.aiStudio.cli.deleteSessions', cliSessionItems, { skipConfirmation: true });
		}
	}
	...
	private async _createNewSubsequentChat(sessionId: string): Promise<IChat> {
		...
		if (chat.sessionType !== CopilotCLISessionType.id) {
			throw new Error('Multiple chats per session is only supported for AI Studio CLI sessions');
		}
