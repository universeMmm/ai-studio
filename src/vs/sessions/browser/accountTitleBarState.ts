...
function getCopilotPresentation(
	...
	if (sentiment.disabled || sentiment.untrusted) {
		return {
			...
			kind: 'warning',
			icon: Codicon.account,
			label: localize('aiStudioUnavailable', "AI Studio Unavailable"),
			ariaLabel: sentiment.untrusted
				? localize('aiStudioUnavailableUntrustedAria', "AI Studio is unavailable in untrusted workspaces")
				: localize('aiStudioUnavailableDisabledAria', "AI Studio is disabled"),
		};
	}
	...
	if (entitlement === ChatEntitlement.Free && (chatQuotaExceeded || completionsQuotaExceeded)) {
		return {
			...
			kind: 'warning',
			icon: Codicon.account,
			label: localize('aiStudioQuotaReached', "Quota Reached"),
			dotBadge: 'error',
	...
	if (entitlement === ChatEntitlement.Free && typeof remainingPercent === 'number' && remainingPercent <= 25) {
		return {
			...
			icon: Codicon.account,
			label: localize('aiStudioTokensRemaining', "Tokens Remaining"),
			badge: `${remainingPercent}%`,
			dotBadge: remainingPercent <= 10 ? 'error' : 'warning',
			ariaLabel: localize('aiStudioTokensRemainingAria', "{0}% AI Studio tokens remaining", remainingPercent),
		};
	}
...
function getQuotaReachedAriaLabel(chatQuotaExceeded: boolean, completionsQuotaExceeded: boolean): string {
	if (chatQuotaExceeded && completionsQuotaExceeded) {
		return localize('aiStudioAllQuotaReachedAria', "AI Studio chat and inline suggestion quota reached");
	}

	if (chatQuotaExceeded) {
		return localize('aiStudioChatQuotaReachedAria', "AI Studio chat quota reached");
	}

	return localize('aiStudioCompletionsQuotaReachedAria', "AI Studio inline suggestion quota reached");
}
