 ...
 * reused at `tool_result` time. Mirrors AI Studio SDK's `IToolStartInfo`
 * shape: AI Studio SDK stashes it at `tool.execution_start` (where the
 * AI Studio SDK hands over complete args); Claude stashes it at the
 ...
 *
 * Mirror of AI Studio SDK's `pendingTools: Map<toolCallId, IToolStartInfo>`
 * pattern in
 * [`mapSessionEvents.ts`](../copilot/mapSessionEvents.ts) — only the
 * seam differs (AI Studio SDK's SDK hands over complete args at
