...
import {
	...
	AgentHostApiError,
	IAgentHostApiService,
	type IAgentHostApiServiceRequestOptions,
 ...
 * side and {@link IAgentHostApiService} on the outbound side. The Claude
...
export class ClaudeProxyService implements IClaudeProxyService {
	...
	private async _sendNonStreamingMessage(
		...
		const options: IAgentHostApiServiceRequestOptions = { headers, signal: entry.ac.signal };
		...
		try {
			...
			while (true) {
				...
				try {
					...
					const envelope = err instanceof AgentHostApiError
						? err.envelope
	...
	private _writeUpstreamErrorResponse(res: http.ServerResponse, err: unknown): void {
		...
		if (err instanceof AgentHostApiError) {
...
/**
 * Build the headers we forward to {@link IAgentHostApiService.messages}
