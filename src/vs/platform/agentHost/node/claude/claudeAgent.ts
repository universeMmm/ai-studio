...
import { projectFromAgentContext } from '../copilot/copilotGitProject.js';
import { IAgentHostApiService } from '../shared/copilotApiService.js';
 ...
 * - {@link models} observable derived from {@link IAgentHostApiService.models}
...
export class ClaudeAgent extends Disposable implements IAgent {
	...
	constructor(
		...
		@IAgentHostApiService private readonly _copilotApiService: IAgentHostApiService,
	...
	async createSession(config: IAgentCreateSessionConfig = {}): Promise<IAgentCreateSessionResult> {
		...
		const project = config.workingDirectory
			? await projectFromAgentContext({ cwd: config.workingDirectory.fsPath }, this._gitService)
			: undefined;
	...
	private async _resumeSession(sessionId: string, sessionUri: URI): Promise<ClaudeAgentSession> {
		...
		try {
			project = await projectFromAgentContext({ cwd: workingDirectory.fsPath }, this._gitService);
		} catch (err) {
