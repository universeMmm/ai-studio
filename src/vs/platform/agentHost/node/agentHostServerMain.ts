...
import { AgentHostAgent } from './copilot/copilotAgent.js';
import { AgentHostApiService, IAgentHostApiService } from './shared/copilotApiService.js';
...
async function main(): Promise<void> {
	...
	if (!options.quiet) {
		...
		diServices.set(IAgentHostGitService, gitService);
		// Register `IAgentHostApiService` BEFORE `IClaudeProxyService` —
		// the proxy service constructor requires it.
		const copilotApiService = instantiationService.createInstance(AgentHostApiService, undefined);
		diServices.set(IAgentHostApiService, copilotApiService);
		...
		const copilotAgent = disposables.add(instantiationService.createInstance(AgentHostAgent));
		agentService.registerProvider(copilotAgent);
		log('AgentHostAgent registered');
