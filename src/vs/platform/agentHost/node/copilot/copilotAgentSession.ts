 ...
 * translates between the two views in {@link AgentHostAgentSession.send}
 * and the `session.mode_changed` listener.
 */
export type AgentHostSdkMode = 'interactive' | 'plan' | 'autopilot';
type AgentHostSdkAttachment = Required<MessageOptions>['attachments'][number];
...
function getAgentHostCLISessionStateDir(userHome: string): string {
 ...
 */
const AGENT_HOST_SDK_TOOL_OUTPUT_BASENAME_RE = /^(?:\d{10,}-copilot-tool-output-[a-z0-9]{6}|copilot-tool-output-\d{10,}-[a-z0-9]{6})\.txt$/i;

function isAgentHostSdkToolOutputTempFile(filePath: string, tmpDir: string): boolean {
	...
	return AGENT_HOST_SDK_TOOL_OUTPUT_BASENAME_RE.test(basename);
}
...
/**
 * Factory function that produces a {@link AgentHostSessionWrapper}.
 * Called by {@link AgentHostAgentSession.initializeSession} with the
...
}) => Promise<AgentHostSessionWrapper>;

/**
 * Options for constructing a {@link AgentHostAgentSession}.
 */
export interface IAgentHostAgentSessionOptions {
 ...
 */
export class AgentHostAgentSession extends Disposable {
	...
	constructor(
		options: IAgentHostAgentSessionOptions,
	...
	resetTurnState(turnId: string): void {
		this._turnId = turnId;
		this._turnAgentHostUsageTotalNanoAiu = 0;
	...
	async send(prompt: string, attachments?: readonly MessageAttachment[], turnId?: string, mode?: AgentHostSdkMode): Promise<void> {
		if (turnId) {
			this._turnId = turnId;
			this._turnAgentHostUsageTotalNanoAiu = 0;
		}
	 ...
	 */
	private async _toSdkAttachment(attachment: MessageAttachment): Promise<AgentHostSdkAttachment | undefined> {
	 ...
	 */
	async applyMode(mode: AgentHostSdkMode | undefined): Promise<void> {
	...
	async handlePermissionRequest(
		...
		try {
			...
			if (request.kind === 'read' && typeof request.path === 'string') {
				if (isAgentHostSdkToolOutputTempFile(request.path, this._environmentService.tmpDir.fsPath)) {
	...
	private _getInternalSessionResourcePath(request: ITypedPermissionRequest): string | undefined {
		...
		const sessionStateDir = normalizePath(URI.file(getAgentHostCLISessionStateDir(this._environmentService.userHome.fsPath)));
	...
	private _subscribeToEvents(): void {
		...
		this._register(wrapper.onUsage(e => {
			...
			if (typeof e.data.copilotUsage?.totalNanoAiu === 'number') {
				this._turnAgentHostUsageTotalNanoAiu += e.data.copilotUsage.totalNanoAiu;
				metadata.copilotUsage = {
					...e.data.copilotUsage,
					totalNanoAiu: this._turnAgentHostUsageTotalNanoAiu,
				};
			}
			this._logService.trace(`[Copilot:${sessionId}] Usage: model=${e.data.model}, in=${e.data.inputTokens ?? '?'}, out=${e.data.outputTokens ?? '?'}, cacheRead=${e.data.cacheReadTokens ?? '?'}, cost=${e.data.cost ?? '?'}, totalNanoAiu=${this._turnAgentHostUsageTotalNanoAiu || '?'}`);
	 ...
	 */
	private _syncAhpConfigFromSdkMode(sdkMode: AgentHostSdkMode): void {
