...
import { AgentHostAgentSession, SessionWrapperFactory, type AgentHostSdkMode, type IActiveClientSnapshot } from './copilotAgentSession.js';
import { IAgentHostSessionContext, projectFromAgentContext } from './copilotGitProject.js';
...
import { AgentHostSessionWrapper } from './copilotSessionWrapper.js';
...
import { AgentHostSlashCommandCompletionProvider } from './copilotSlashCommandCompletionProvider.js';
...
export const COPILOT_AGENT_HOST_SYSTEM_MESSAGE = {
	...
	sections: {
		identity: {
			action: 'replace',
			content: 'You are an AI assistant using AI Studio CLI runtime in VS Code. You help users with software engineering tasks. When asked about your identity, you must state that you are an AI assistant using AI Studio CLI runtime in VS Code.',
		},
	},
...
export function getAgentHostWorktreesRoot(repositoryRoot: URI): URI {
	return URI.joinPath(repositoryRoot, '..', `${basename(repositoryRoot.fsPath)}.worktrees`);
}

export function getAgentHostWorktreeName(branchName: string): string {
	return branchName.replace(/\//g, '-');
}

export function getAgentHostWorktreeBranchName(sessionId: string, branchNameHint: string | undefined): string {
 ...
 * branch name.
 */
export function getAgentHostBranchNameHintFromMessage(message: string): string | undefined {
	const words = message
		.toLowerCase()
		.normalize('NFKD')
		.replace(/[^a-z0-9]+/g, '-')
 ...
 */
export class AgentHostAgent extends Disposable implements IAgent {
	readonly id = 'aiStudioCli' as const;
	...
	private readonly _sessions = this._register(new DisposableMap<string, AgentHostAgentSession>());
	/**
	...
	constructor(
		...
		this._register(completions.registerProvider(new AgentHostSlashCommandCompletionProvider(this.id, {
	...
	protected _createAgentHostClient(options: CopilotClientOptions): CopilotClient {
	...
	getDescriptor(): IAgentDescriptor {
		return {
			provider: 'aiStudioCli',
			displayName: 'AI Studio CLI',
			description: 'AI Studio SDK agent running in a dedicated process',
		};
	}
	...
	protected _enablePlanModeOnClient(client: CopilotClient): void {
		...
		// hands the request off to the matching {@link AgentHostAgentSession},
	...
	private async _ensureClient(): Promise<CopilotClient> {
		...
		const clientStarting = (async () => {
			...
			const client = this._createAgentHostClient(clientOptions);
			await client.start();
			if (this._githubToken !== tokenAtStartup) {
				await client.stop();
				throw new Error('AI Studio authentication changed while the client was starting');
			}
	...
	private _createThinkingLevelConfigSchema(supportedReasoningEfforts: readonly string[] | undefined, defaultReasoningEffort: string | undefined): ConfigSchema | undefined {
		...
		const enumLabels = supportedReasoningEfforts.map(value => {
			switch (value) {
				case 'low': return localize('aiStudio.modelThinkingLevel.low', "Low");
				case 'medium': return localize('aiStudio.modelThinkingLevel.medium', "Medium");
				case 'high': return localize('aiStudio.modelThinkingLevel.high', "High");
				case 'xhigh': return localize('aiStudio.modelThinkingLevel.xhigh', "Extra High");
		...
		return {
			...
			properties: {
				[ThinkingLevelConfigKey]: {
					type: 'string',
					title: localize('aiStudio.modelThinkingLevel.title', "Thinking Level"),
					description: localize('aiStudio.modelThinkingLevel.description', "Controls how much reasoning effort the model uses."),
	...
	async createSession(config?: IAgentCreateSessionConfig): Promise<IAgentCreateSessionResult> {
		...
		if (config?.fork) {
			...
			return this._sessionSequencer.queue(sourceSessionId, async () => {
				...
				const project = await projectFromAgentContext({ cwd: config.workingDirectory?.fsPath }, this._gitService);
	 ...
	 * `isolation === 'worktree'`), instantiates the {@link AgentHostAgentSession},
	 ...
	 */
	private async _materializeProvisional(sessionId: string, prompt: string): Promise<AgentHostAgentSession> {
		const provisional = this._provisionalSessions.get(sessionId);
		if (!provisional) {
		...
		const factory: SessionWrapperFactory = async callbacks => {
			...
			return new AgentHostSessionWrapper(raw);
		};

		let agentSession: AgentHostAgentSession;
		try {
		...
		const project = await projectFromAgentContext({ cwd: workingDirectory?.fsPath }, this._gitService);
	...
	async sendMessage(session: URI, prompt: string, attachments?: readonly MessageAttachment[], turnId?: string): Promise<void> {
		...
		await this._sessionSequencer.queue(sessionId, async () => {
			...
			let entry: AgentHostAgentSession | undefined;
	 ...
	 */
	private _resolveSdkMode(session: URI): AgentHostSdkMode | undefined {
	...
	/**
	 * Creates a {@link AgentHostAgentSession}, registers it in the sessions map,
	 * and returns it. The caller must call {@link AgentHostAgentSession.initializeSession}
	 * to wire up the SDK session.
	 */
	private _createAgentSession(wrapperFactory: SessionWrapperFactory, sessionId: string, shellManager: ShellManager, workingDirectory: URI | undefined, customizationDirectory: URI | undefined, snapshot?: IActiveClientSnapshot): AgentHostAgentSession {
		const sessionUri = AgentSession.uri(this.id, sessionId);
	...
	protected async _resumeSession(sessionId: string): Promise<AgentHostAgentSession> {
	...
	private async _getBranches(workingDirectory: URI, query?: string): Promise<string[]> {
		return this._gitService.getBranches(workingDirectory, { query, limit: AgentHostAgent._BRANCH_COMPLETION_LIMIT });
	}
	...
	protected async _resolveSessionWorkingDirectory(config: IAgentCreateSessionConfig | undefined, sessionId: string, prompt?: string): Promise<URI | undefined> {
		...
		const worktreesRoot = getAgentHostWorktreesRoot(repositoryRoot);
		const branchNameHint = prompt ? getAgentHostBranchNameHintFromMessage(prompt) : undefined;
		const branchName = getAgentHostWorktreeBranchName(sessionId, branchNameHint);
		const worktree = URI.joinPath(worktreesRoot, getAgentHostWorktreeName(branchName));
	...
	private async _writeWorktreeMetadata(session: URI, metadata: { branchName: string; baseBranch: string | undefined; worktreePath: URI; repositoryRoot: URI }): Promise<void> {
		...
		try {
			const work: Promise<void>[] = [
				dbRef.object.setMetadata(AgentHostAgent._META_WORKTREE_BRANCH, metadata.branchName),
				dbRef.object.setMetadata(AgentHostAgent._META_WORKTREE_PATH, metadata.worktreePath.toString()),
				dbRef.object.setMetadata(AgentHostAgent._META_WORKTREE_REPOSITORY_ROOT, metadata.repositoryRoot.toString()),
			];
	...
	private async _readWorktreeMetadata(session: URI): Promise<{ branchName: string; worktreePath?: URI; repositoryRoot?: URI } | undefined> {
		...
		try {
			const [branchName, worktreePathRaw, repositoryRootRaw] = await Promise.all([
				ref.object.getMetadata(AgentHostAgent._META_WORKTREE_BRANCH),
				ref.object.getMetadata(AgentHostAgent._META_WORKTREE_PATH),
				ref.object.getMetadata(AgentHostAgent._META_WORKTREE_REPOSITORY_ROOT),
			]);
			if (!branchName) {
				return undefined;
			}
	...
	private async _storeSessionMetadata(session: URI, model: ModelSelection | undefined, workingDirectory: URI | undefined, customizationDirectory: URI | undefined, project: IAgentSessionProjectInfo | undefined, projectResolved = project !== undefined): Promise<void> {
		...
		try {
			const work: Promise<void>[] = [];
			if (model) {
				work.push(db.setMetadata(AgentHostAgent._META_MODEL, this._serializeModelSelection(model)));
			}
			if (workingDirectory) {
				work.push(db.setMetadata(AgentHostAgent._META_CWD, workingDirectory.toString()));
			}
			if (customizationDirectory) {
				work.push(db.setMetadata(AgentHostAgent._META_CUSTOMIZATION_DIRECTORY, customizationDirectory.toString()));
			}
			if (projectResolved) {
				work.push(db.setMetadata(AgentHostAgent._META_PROJECT_RESOLVED, 'true'));
			}
			if (project) {
				work.push(db.setMetadata(AgentHostAgent._META_PROJECT_URI, project.uri.toString()));
				work.push(db.setMetadata(AgentHostAgent._META_PROJECT_DISPLAY_NAME, project.displayName));
			}
			await Promise.all(work);
		} finally {
			dbRef.dispose();
	...
	private async _readSessionMetadata(session: URI): Promise<{ model?: ModelSelection; agent?: AgentSelection; workingDirectory?: URI; customizationDirectory?: URI }> {
		...
		try {
			const [model, agent, cwd, customizationDirectory] = await Promise.all([
				ref.object.getMetadata(AgentHostAgent._META_MODEL),
				ref.object.getMetadata(AgentHostAgent._META_AGENT),
				ref.object.getMetadata(AgentHostAgent._META_CWD),
				ref.object.getMetadata(AgentHostAgent._META_CUSTOMIZATION_DIRECTORY),
			]);
			return {
	...
	private async _readStoredSessionMetadata(session: URI): Promise<{ model?: ModelSelection; agent?: AgentSelection; workingDirectory?: URI; customizationDirectory?: URI; project?: IAgentSessionProjectInfo; resolved: boolean } | undefined> {
		...
		try {
			const [model, agent, cwd, customizationDirectory, resolved, uri, displayName] = await Promise.all([
				...
				ref.object.getMetadata(AgentHostAgent._META_CWD),
				ref.object.getMetadata(AgentHostAgent._META_CUSTOMIZATION_DIRECTORY),
				ref.object.getMetadata(AgentHostAgent._META_PROJECT_RESOLVED),
				ref.object.getMetadata(AgentHostAgent._META_PROJECT_URI),
				ref.object.getMetadata(AgentHostAgent._META_PROJECT_DISPLAY_NAME),
			]);
	...
	private async _storeSessionAgentMetadata(session: URI, agent: AgentSelection | undefined): Promise<void> {
		...
		try {
			...
			await dbRef.object.setMetadata(AgentHostAgent._META_AGENT, agent ? this._serializeAgentSelection(agent) : '');
		} finally {
			dbRef.dispose();
		}
	}
	...
	private _resolveSessionProject(context: IAgentHostSessionContext | undefined, limiter: Limiter<IAgentSessionProjectInfo | undefined>, projectByContext: Map<string, Promise<IAgentSessionProjectInfo | undefined>>): Promise<IAgentSessionProjectInfo | undefined> {
		...
		if (!project) {
			project = limiter.queue(() => projectFromAgentContext(context, this._gitService));
			projectByContext.set(key, project);
		}
		return project;
	}

	private _projectContextKey(context: IAgentHostSessionContext | undefined): string | undefined {
		if (context?.cwd) {
		...
		try {
			...
			this._resolved = {
				customization: {
					...
					statusMessage: plugin ? undefined : localize('aiStudioAgent.pluginParseError', "Error parsing plugin."),
		...
		if (!parsed) {
			return {
				customization: {
					...
					statusMessage: localize('aiStudioAgent.pluginParseError', "Error parsing plugin."),
				},
			};
		}
