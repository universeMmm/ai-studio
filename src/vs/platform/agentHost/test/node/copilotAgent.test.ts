...
import { COPILOT_AGENT_HOST_SYSTEM_MESSAGE, AgentHostAgent, getAgentHostBranchNameHintFromMessage, getAgentHostWorktreeBranchName, getAgentHostWorktreeName, getAgentHostWorktreesRoot } from '../../node/copilot/copilotAgent.js';
import { NULL_CHECKPOINT_SERVICE } from '../../common/agentHostCheckpointService.js';
import { AgentHostAgentSession, type SessionWrapperFactory } from '../../node/copilot/copilotAgentSession.js';
import { AgentHostSessionWrapper } from '../../node/copilot/copilotSessionWrapper.js';
...
class TestableAgentHostAgent extends AgentHostAgent {
	...
	protected override _createAgentHostClient(): CopilotClient {
	...
	protected override async _resumeSession(sessionId: string): Promise<AgentHostAgentSession> {
...
function createTestAgentContext(disposables: Pick<DisposableStore, 'add'>, options?: { sessionDataService?: ISessionDataService; copilotClient?: ITestCopilotClient; gitService?: TestAgentHostGitService; environmentServiceRegistration?: 'native' | 'none'; pluginManager?: IAgentPluginManager; fileService?: FileService }): { agent: AgentHostAgent; instantiationService: IInstantiationService; configurationService: IAgentConfigurationService; fileService: FileService } {
	...
	const agent = options?.copilotClient
		? instantiationService.createInstance(TestableAgentHostAgent, options.copilotClient)
		: instantiationService.createInstance(AgentHostAgent);
	return { agent, instantiationService, configurationService: configService, fileService };
}

function createTestAgent(disposables: Pick<DisposableStore, 'add'>, options?: { sessionDataService?: ISessionDataService; copilotClient?: ITestCopilotClient; gitService?: TestAgentHostGitService; environmentServiceRegistration?: 'native' | 'none'; pluginManager?: IAgentPluginManager }): AgentHostAgent {
	return createTestAgentContext(disposables, options).agent;
}

function createAgentSessionThroughAgent(agent: AgentHostAgent, instantiationService: IInstantiationService): AgentHostAgentSession {
	...
	const wrapperFactory: SessionWrapperFactory = async () => new AgentHostSessionWrapper(new MockCopilotSession() as unknown as CopilotSession);
	return (agent as unknown as {
		_createAgentSession: (wrapperFactory: SessionWrapperFactory, sessionId: string, shellManager: ShellManager) => AgentHostAgentSession;
...
async function disposeAgent(agent: AgentHostAgent): Promise<void> {
	await agent.shutdown();
	agent.dispose();
	// AgentHostAgent.dispose calls super.dispose() from a promise continuation so
...
suite('AgentHostAgent', () => {
	const disposables = ensureNoDisposablesAreLeakedInTestSuite();

	test('uses the Copilot CLI sibling worktrees root convention', () => {
		assert.strictEqual(
			getAgentHostWorktreesRoot(URI.file('/Users/me/src/vscode')).fsPath,
			URI.file('/Users/me/src/vscode.worktrees').fsPath,
		);
	});

	test('uses Agents-window Copilot CLI branch prefix', () => {
		assert.strictEqual(getAgentHostWorktreeBranchName('12345678-aaaa-bbbb-cccc-123456789abc', 'add-agent-host-config'), 'agents/add-agent-host-config-12345678');
		assert.strictEqual(getAgentHostWorktreeBranchName('12345678-aaaa-bbbb-cccc-123456789abc', undefined), 'agents/12345678-aaaa-bbbb-cccc-123456789abc');
	});

	test('uses Git extension branch-derived worktree folder names', () => {
		assert.strictEqual(getAgentHostWorktreeName('agents/add-agent-host-config-12345678'), 'agents-add-agent-host-config-12345678');
	});

	test('keeps hinted branch names short', () => {
		assert.strictEqual(getAgentHostWorktreeBranchName('12345678-aaaa-bbbb-cccc-123456789abc', 'a'.repeat(48)).length, 'agents/'.length + 48 + '-12345678'.length);
	});

	test('derives slug branch hint from first message', () => {
		assert.strictEqual(getAgentHostBranchNameHintFromMessage('Add agent host config'), 'add-agent-host-config');
		assert.strictEqual(getAgentHostBranchNameHintFromMessage('  Fix: the bug!! '), 'fix-the-bug');
		assert.strictEqual(getAgentHostBranchNameHintFromMessage('Refactor café ☕ rendering'), 'refactor-cafe-rendering');
		assert.strictEqual(getAgentHostBranchNameHintFromMessage('one two three four five six seven eight nine ten'), 'one-two-three-four-five-six-seven-eight');
		assert.strictEqual(getAgentHostBranchNameHintFromMessage('a'.repeat(100))?.length, 48);
		assert.strictEqual(getAgentHostBranchNameHintFromMessage('!!! ??? ...'), undefined);
		assert.strictEqual(getAgentHostBranchNameHintFromMessage(''), undefined);
	});
	...
	suite('onClientToolCallComplete', () => {
		 ...
		 */
		function installStubSession(agent: AgentHostAgent, sessionId: string): { calls: { toolCallId: string; result: ToolCallResult }[] } {
	...
	suite('worktree announcement', () => {
		...
		// stubbed AgentHostAgentSession is injected via overriding _resumeSession
		...
		test('emits announcement live as a delta on first sendMessage and persists it for restore via getSessionMessages', async () => {
			...
			try {
				...
				const expectedBranchName = getAgentHostWorktreeBranchName(sessionId, branchHint);
