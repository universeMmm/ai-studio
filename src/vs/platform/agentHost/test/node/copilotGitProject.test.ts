...
import { projectFromAgentContext, projectFromRepository, resolveGitProject } from '../../node/copilot/copilotGitProject.js';
...
suite('Copilot Git Project', () => {
	...
	test('falls back to repository context when no git project is available', async () => {
		...
		const project = await projectFromAgentContext({ repository: 'microsoft/vscode' }, gitService);
