...
export const enum AgentHostConfigKey {
	...
	/** When true, AI Studio SDK sessions use the SDK's default terminal behavior instead of Agent Host's terminal tool override. */
...
export const agentHostCustomizationConfigSchema = createSchema({
	...
	[AgentHostConfigKey.DisableCustomTerminalTool]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.disableCustomTerminalTool.description', "When enabled, AI Studio SDK sessions use the SDK's default terminal behavior instead of Agent Host's terminal tool override."),
		default: false,
	}),
});
