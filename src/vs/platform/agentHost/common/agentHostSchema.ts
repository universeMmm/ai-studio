...
export const platformSessionSchema = createSchema({
	[SessionConfigKey.AutoApprove]: schemaProperty<AutoApproveLevel>({
		...
		enumDescriptions: [
			localize('agentHost.sessionConfig.autoApprove.defaultDescription', "AI Studio uses your configured settings"),
...
export const platformRootSchema = createSchema({
	...
	[AgentHostSessionSyncEnabledConfigKey]: schemaProperty<boolean>({
		...
		description: localize('agentHost.config.sessionSyncEnabled.description', "Whether remote session sync is enabled for the AI Studio CLI."),
		default: false,
	}),
});
